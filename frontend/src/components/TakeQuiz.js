...
function TakeQuiz() {
    ...
    useEffect(() => {
        const fetchQuizzes = async () => {
            try {
                const response = await axios.get('http://localhost:5001/quizzes'); // Ensure the endpoint is /quizzes
    ...
    const handleQuizSelect = async (e) => {
        ...
        try {
            const response = await axios.get(`http://localhost:5001/quiz/${quizId}`); // Fetch the selected quiz details
        ...
        React.createElement('div', { className: 'p-4' },
            ...
            quizzes.length > 0 ? (
                React.createElement('select', {
                    ...
                    React.createElement('option', { value: '', disabled: true }, 'Select a quiz'),
                    quizzes.map((quiz) => (
                        React.createElement('option', { key: quiz._id, value: quiz._id }, quiz.title)
                    ))
                )
            ) : (
            ...
            selectedQuiz && React.createElement('div', null,
                React.createElement('h2', { className: 'text-xl mb-2' }, selectedQuiz.title),
                selectedQuiz.questions.map((q, qIndex) => (
                    React.createElement('div', { key: qIndex, className: 'my-4' },
                        React.createElement('p', { className: 'font-semibold' }, q.questionText),
                        q.options.map((opt, optIndex) => (
                            React.createElement('label', { key: optIndex, className: 'block' },
