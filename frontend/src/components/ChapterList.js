...
const ChapterList = ({
  ...
  const fetchChapters = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5005/api/chapters/subject/${subject._id}`
      );
  ...
  const handleAddChapter = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5005/api/chapters", {
        name: newChapterName,
        subject: subject._id,
      });
  ...
  const handleEditChapter = async () => {
    if (newChapterName.trim() && editingChapter) {
      try {
        await axios.put(
          `http://localhost:5005/api/chapters/${editingChapter._id}`,
          {
            name: newChapterName,
          }
        );
  ...
  const handleDeleteChapter = async (chapterId) => {
    if (window.confirm("Are you sure you want to delete this chapter?")) {
      try {
        await axios.delete(`http://localhost:5005/api/chapters/${chapterId}`);
        fetchChapters();
      } catch (error) {
        ...
        {filteredChapters.map((chapter) => (
          <div key={chapter._id} className="chapter-card">
            ...
            <div className="chapter-actions">
              ...
              <button
                className={`action-button ${
                  chapter.quizzes?.length ? "has-quizzes" : ""
                }`}
          ...
          <div className="modal">
            ...
            <div className="modal-actions">
              <button
                className="action-button"
                onClick={
                  isEditingChapter ? handleEditChapter : handleAddChapter
                }
              >
